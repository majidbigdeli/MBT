...
namespace SimlifiedPhoneBookClient
{
    class Program
    {
        static void Main()
        {
            ...
            var client = MbtServiceClientBuilder.CreateClient<IPhoneBookService>(new MbtTcpEndPoint("127.0.0.1", 10048));

            //Directly call a method (it automatically connects, calls and disconnects)
            client.ServiceProxy.AddPerson(new PhoneBookRecord { Name = "Majid Bigdeli", Phone = "5881112233" });
