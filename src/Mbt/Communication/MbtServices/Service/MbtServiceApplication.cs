...
namespace Hik.Communication.MbtServices.Service
{
    ...
    /// </summary>
    internal class MbtServiceApplication : IMbtServiceApplication
    {
        ...
        /// </summary>
        private readonly IMbtServer _mbtServer;
        ...
        /// </summary>
        private readonly ThreadSafeSortedList<long, IMbtServiceClient> _serviceClients;
        ...
        /// <exception cref="ArgumentNullException">Throws ArgumentNullException if mbtServer argument is null</exception>
        public MbtServiceApplication(IMbtServer mbtServer)
        {
            if (mbtServer == null)
            {
                throw new ArgumentNullException("mbtServer");
            }

            _mbtServer = mbtServer;
            _mbtServer.ClientConnected += MbtServer_ClientConnected;
            _mbtServer.ClientDisconnected += MbtServer_ClientDisconnected;
            _serviceObjects = new ThreadSafeSortedList<string, ServiceObject>();
            _serviceClients = new ThreadSafeSortedList<long, IMbtServiceClient>();
        }
        ...
        public void AddService<TServiceInterface, TServiceClass>(TServiceClass service)
            where TServiceClass : MbtService, TServiceInterface
        ...
        private void MbtServer_ClientConnected(object sender, ServerClientEventArgs e)
        {
            var requestReplyMessenger = new RequestReplyMessenger<IMbtServerClient>(e.Client);
            requestReplyMessenger.MessageReceived += Client_MessageReceived;
            requestReplyMessenger.Start();

            var serviceClient = MbtServiceClientFactory.CreateServiceClient(e.Client, requestReplyMessenger);
        ...
        /// <param name="sender">Source of event</param>
        /// <param name="e">Event arguments</param>
        private void MbtServer_ClientDisconnected(object sender, ServerClientEventArgs e)
        {
        ...
        private void Client_MessageReceived(object sender, MessageEventArgs e)
        {
            ...
            var requestReplyMessenger = (RequestReplyMessenger<IMbtServerClient>)sender;
            ...
            try
            {
                ...
                {
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new MbtRemoteException("There is no service with name '" + invokeMessage.ServiceClassName + "'"));
                    return;
                }
                ...
                catch (TargetInvocationException ex)
                {
                    ...
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new MbtRemoteException(innerEx.Message + Environment.NewLine + "Service Version: " + serviceObject.ServiceAttribute.Version, innerEx));
                    return;
                }
                catch (Exception ex)
                {
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new MbtRemoteException(ex.Message + Environment.NewLine + "Service Version: " + serviceObject.ServiceAttribute.Version, ex));
                    return;
                }
            }
            catch (Exception ex)
            {
                SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new MbtRemoteException("An error occured during remote service method call.", ex));
                return;
            }
        }
        ...
        private static void SendInvokeResponse(IMessenger client, IMbtMessage requestMessage, object returnValue, MbtRemoteException exception)
        {
            try
            {
                client.SendMessage(
        ...
        private void OnClientConnected(IMbtServiceClient client)
        {
        ...
        /// </summary>
        /// <param name="client"></param>
        private void OnClientDisconnected(IMbtServiceClient client)
        {
        ...
        private sealed class ServiceObject
        {
            ...
            /// </summary>
            public MbtService Service { get; private set; }

            /// <summary>
            /// MbtService attribute of Service object's class.
            /// </summary>
            public MbtServiceAttribute ServiceAttribute { get; private set; }
            ...
            public ServiceObject(Type serviceInterfaceType, MbtService service)
            {
                Service = service;
                var classAttributes = serviceInterfaceType.GetCustomAttributes(typeof(MbtServiceAttribute), true);
                if (classAttributes.Length <= 0)
                {
                    throw new Exception("Service interface (" + serviceInterfaceType.Name + ") must has MbtService attribute.");
                }

                ServiceAttribute = classAttributes[0] as MbtServiceAttribute;
